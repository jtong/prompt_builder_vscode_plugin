...
├── package-lock.json
├── package.json
├── prompt-context-builder-plugin-0.1.7.vsix
└── webpack.config.js
...
我希望 添加一个命令，他可以发送一个HTTP请求去指定的URL，这个指定的URL是可以在setting里设置的，而且可以设置多个预选值，然后指定其中一个作为确定的访问URL。