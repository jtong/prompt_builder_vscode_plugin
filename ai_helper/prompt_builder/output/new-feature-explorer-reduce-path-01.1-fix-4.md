...
目前存在问题，当有多级子目录都只有一个下级子目录的时候，能显示正确，但会报错。

比如当项目根路径是'/Users/jtong/folder_reduce'，其内部结构如下的时候：
...
```
ENOENT: no such file or directory, scandir '/Users/jtong/folder_reduce/src/abc'
```

应该是拼接后漏掉了中间的路径，实际上应该被打开的文件夹是'/Users/jtong/folder_reduce/src/aaa/abc'
